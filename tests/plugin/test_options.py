...
class TestResolve(unittest.TestCase):
    def test_defaults(self):
        opts = PageOptions.resolve({}, {"type": "openapi"})
    ...
    def test_site_defaults_apply_when_page_is_silent(self):
        opts = PageOptions.resolve({"hide_bindings": True},
                                   {"type": "asyncapi", "spec": "a.yml"})
        self.assertTrue(opts.hide_bindings)

    def test_unknown_keys_are_ignored(self):
        opts = PageOptions.resolve({}, {"type": "openapi", "spec": "a.yml", "nonsense": 1})
        self.assertEqual(opts.spec, "a.yml")
...
class TestCoercion(unittest.TestCase):
    def test_schema_depth_clamped_and_defaulted(self):
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "schema_depth": 0}).schema_depth, 1)
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "schema_depth": -4}).schema_depth, 1)
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "schema_depth": "x"}).schema_depth, 3)
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "schema_depth": "7"}).schema_depth, 7)

    def test_schema_depth_rejects_bool(self):
        """`isinstance(True, int)` holds, so `schema_depth: true` must not become depth 1."""
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "schema_depth": True}).schema_depth, 3)
    ...
    def test_quoted_false_is_falsey(self):
        ...
        for value in ("false", "False", " no ", "off", "0", ""):
            with self.subTest(value=value):
                self.assertFalse(PageOptions.resolve({}, {"type": "openapi", "hide_internal": value}).hide_internal)

    def test_truthy_strings(self):
        for value in ("true", "TRUE", "yes", "on", "1"):
            with self.subTest(value=value):
                self.assertTrue(PageOptions.resolve({}, {"type": "openapi", "hide_internal": value}).hide_internal)

    def test_unparseable_bool_falls_back(self):
        self.assertFalse(PageOptions.resolve({}, {"type": "openapi", "hide_internal": "maybe"}).hide_internal)
        self.assertFalse(PageOptions.resolve({}, {"type": "openapi", "hide_internal": None}).hide_internal)

    def test_strings_are_stripped(self):
        opts = PageOptions.resolve({}, {"type": "openapi", "spec": "  a.yml  ", "title": " T "})
        self.assertEqual(opts.spec, "a.yml")
        self.assertEqual(opts.title, "T")

    def test_attachments_parsed_to_dataclasses(self):
        self.assertEqual(
            PageOptions.resolve({}, {"type": "openapi", "attachments": ["a", "b"]}).attachments,
            (Attachment(path="a"), Attachment(path="b")))

    def test_attachment_mapping_form(self):
        opts = PageOptions.resolve({}, {"type": "openapi", "attachments": [
    ...
    def test_attachment_mapping_defaults(self):
        opts = PageOptions.resolve({}, {"type": "openapi", "attachments": [{"path": "x.proto"}]})
    ...
    def test_attachments_not_a_list(self):
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "attachments": "nope"}).attachments, ())
        self.assertEqual(PageOptions.resolve({}, {"type": "openapi", "attachments": None}).attachments, ())
...
class TestType(unittest.TestCase):
    ...
    def test_missing_type_raises(self):
        """Without `type:` there is no renderer to dispatch to."""
        for page_opts in (None, {}, {"type": ""}, {"type": "  "}, {"type": None},
                          {"spec": "a.yml"}):
            with self.subTest(page_opts=page_opts):
                with self.assertRaises(ValueError):
                    PageOptions.resolve({}, page_opts)

    def test_missing_type_message(self):
        with self.assertRaises(ValueError) as ctx:
            PageOptions.resolve()
        self.assertIn("missing required `type:` option", str(ctx.exception))
